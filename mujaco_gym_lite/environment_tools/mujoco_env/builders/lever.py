...
from mujaco_gym_lite.environment_tools.mujoco_env.xmls.metaworld_objects.lever import add_lever
...
@dataclass
class LeverRobotEnvBuilderConfig(RobotEnvBuilderConfig):
    # model settings
    lever_name: str = "lever"
    lever_joint_name: str = "lever_joint"
    lever_body_name: str = "lever_body"
    lever_position: npt.NDArray = np.array([-0.25, 0.0, 0.0])
    lever_rotation: npt.NDArray = np.array([1.0, 0.0, 0.0, 0.0])
    lever_geom_name: str = "lever"
    lever_joint_range: tuple[float, float] = (0.0, np.deg2rad(50.0))
    lever_handle_site_name: str = "lever_handle_site"
    lever_handle_collision_name: str = "lever_handle_collision"
    ...
    end_effector_min_position: npt.NDArray = np.array([-0.2, -0.25, 0.15])
    end_effector_max_position: npt.NDArray = np.array([0.2, 0.1, 0.5])
...
class LeverRobotEnvBuilder(RobotEnvBuilder):
    _config: LeverRobotEnvBuilderConfig

    def __init__(self, config: LeverRobotEnvBuilderConfig):
    ...
    def build_env_model(
        ...
        lever = Lever(
            base_body_name=self._config.lever_body_name,
            lever_joint_names=[self._config.lever_joint_name],
            body_names=[self._config.lever_body_name],
            geom_root_name=self._config.lever_geom_name,
            lever_joint_range=self._config.lever_joint_range,
            lever_handle_geom_name=self._config.lever_handle_collision_name,
            lever_handle_site_name=self._config.lever_handle_site_name,
        )
    ...
    def _build_objects_xml(self, generator: MJCFGenerator):
        add_lever(
            ...
            lever_name=self._config.lever_name,
            lever_position=self._config.lever_position,
            lever_rotation=self._config.lever_rotation,
            lever_body_name=self._config.lever_body_name,
            lever_joint_name=self._config.lever_joint_name,
            lever_handle_geom_name=self._config.lever_handle_collision_name,
            lever_handle_site_name=self._config.lever_handle_site_name,
        )
