...
from mujaco_gym_lite.environment_tools.mujoco_env.xmls.metaworld_objects.assembly_ring import (
    add_assembly_ring,
    add_assembly_rod,
)
...
@dataclass
class AssemblyRingRobotEnvBuilderConfig(RobotEnvBuilderConfig):
    # model settings
    assembly_ring_name: str = "assembly_ring"
    assembly_ring_joint_name: str = "assembly_ring_joint"
    assembly_ring_body_name: str = "assembly_ring_body"
    assembly_ring_position: npt.NDArray = np.array([0.0, -0.1, 0.03])
    assembly_ring_rotation: npt.NDArray = euler_to_quat(np.pi * 0.5, order="z")
    assembly_ring_handle_site_name: str = "assembly_ring_handle_site"
    assembly_ring_site_name: str = "assembly_ring_site"
    obstacle_positions: Optional[tuple[tuple[float, float, float], ...]] = ((-0.5, 0.125, 0.3), (0.75, 0.0, 0.3))
    assembly_ring_geom_name: str = "assembly_ring"

    assembly_rod_name: str = "assembly_rod"
    assembly_rod_body_name: str = "assembly_rod_body"
    assembly_rod_position: npt.NDArray = np.array([-0.15, 0.0, 0.0])
    assembly_rod_rotation: npt.NDArray = np.array([1.0, 0.0, 0.0, 0.0])
    assembly_rod_target_top_site_name: str = "assembly_rod_target_top_site"
    assembly_rod_target_bottom_site_name: str = "assembly_rod_target_bottom_site"

    assembly_insertion_height_threshold: float = 0.05
    assembly_insertion_radius_threshold: float = 0.05
...
class AssemblyRingRobotEnvBuilder(RobotEnvBuilder):
    _config: AssemblyRingRobotEnvBuilderConfig

    def __init__(self, config: AssemblyRingRobotEnvBuilderConfig):
    ...
    def build_env_model(
        ...
        ring = AssemblyRing(
            base_body_name=self._config.assembly_ring_body_name,
            ring_joint_names=[self._config.assembly_ring_joint_name],
            ring_body_names=[self._config.assembly_ring_body_name],
            geom_root_name=self._config.assembly_ring_name,
            ring_site_name=self._config.assembly_ring_site_name,
            ring_handle_site_name=self._config.assembly_ring_handle_site_name,
        )
        rod = AssemblyRod(
            base_body_name=self._config.assembly_rod_body_name,
            assembly_rod_joint_names=[],
            assembly_rod_body_names=[self._config.assembly_rod_body_name],
            geom_root_name=self._config.assembly_rod_name,
            rod_top_site_name=self._config.assembly_rod_target_top_site_name,
            rod_bottom_site_name=self._config.assembly_rod_target_bottom_site_name,
            insertion_height_threshold=self._config.assembly_insertion_height_threshold,
            insertion_radius_threshold=self._config.assembly_insertion_radius_threshold,
        )
    ...
    def _build_objects_xml(self, generator: MJCFGenerator):
        info = add_assembly_ring(
            generator=generator,
            asset_dir_path=self._config.asset_dir_path,
            assembly_ring_name=self._config.assembly_ring_name,
            assembly_ring_rotation=self._config.assembly_ring_rotation,
            assembly_ring_position=self._config.assembly_ring_position,
            assembly_ring_handle_site_name=self._config.assembly_ring_handle_site_name,
            assembly_ring_body_name=self._config.assembly_ring_body_name,
            assembly_ring_joint_name=self._config.assembly_ring_joint_name,
            assembly_ring_site_name=self._config.assembly_ring_site_name,
        )

        add_assembly_rod(
            generator=generator,
            asset_dir_path=self._config.asset_dir_path,
            assembly_rod_name=self._config.assembly_rod_name,
            assembly_rod_rotation=self._config.assembly_rod_rotation,
            assembly_rod_position=self._config.assembly_rod_position,
            assembly_rod_body_name=self._config.assembly_rod_body_name,
            assembly_rod_target_top_site_name=self._config.assembly_rod_target_top_site_name,
            assembly_rod_target_bottom_site_name=self._config.assembly_rod_target_bottom_site_name,
        )
        ...
        if self._config.add_task_area_marker:
            add_workspace_marker(
                generator,
                "task_area_marker_1",
                marker_positions=[np.array([0.0, -0.175, 0.09])],
                rgbas=[np.array([0.0, 0.5, 0.0, 1.0]) for _ in range(1)],
                size=np.array([0.015, 0.015, 0.015]),
                attach_body=info["ring_body"],
            )
            add_workspace_marker(
                generator,
                "task_area_marker_2",
                marker_positions=[np.array([0.0, -0.375, 0.09])],
                rgbas=[np.array([0.0, 0.5, 0.0, 1.0]) for _ in range(1)],
                size=np.array([0.015, 0.015, 0.015]),
                attach_body=info["ring_body"],
            )
            add_workspace_marker(
                generator,
                "task_area_marker_3",
                marker_positions=[np.array([0.0, 0.1, 0.035])],
                rgbas=[np.array([0.0, 0.5, 0.0, 1.0]) for _ in range(1)],
                size=np.array([0.015, 0.015, 0.015]),
                attach_body=info["ring_body"],
            )
            ...
            add_workspace_marker(
                generator,
                "task_area_marker_5",
                marker_positions=[np.array([0.1, 0.0, 0.035])],
                rgbas=[np.array([0.0, 0.5, 0.0, 1.0]) for _ in range(1)],
                size=np.array([0.015, 0.015, 0.015]),
                attach_body=info["ring_body"],
            )
    ...
    def _build_env(
        ...
        env: MujocoEnv = gymnasium.make(  # type: ignore
            ...
            assembly_ring=env_models["assembly_ring"],
            assembly_rod=env_models["assembly_rod"],
            ...
            segmentation_object_names=(
                ...
                self._config.assembly_rod_name,
                self._config.assembly_ring_name,
                "task_area_marker_1",
                "task_area_marker_2",
                "task_area_marker_3",
                "task_area_marker_4",
