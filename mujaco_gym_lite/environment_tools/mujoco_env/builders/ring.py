...
from mujaco_gym_lite.environment_tools.mujoco_env.xmls.metaworld_objects.ring import (
    add_ring,
    add_rod,
)
...
@dataclass
class RingHangingRobotEnvBuilderConfig(RobotEnvBuilderConfig):
    # model settings
    ring_name: str = "ring"
    ring_joint_name: str = "ring_joint"
    ring_body_name: str = "ring_body"
    ring_position: npt.NDArray = np.array([0.0, -0.1, 0.15])
    ring_rotation: npt.NDArray = euler_to_quat([np.pi * 0.5, np.pi * 0.5], order="zx")
    ring_handle_site_name: str = "ring_handle_site"
    ring_site_name: str = "ring_site"
    obstacle_positions: Optional[tuple[tuple[float, float, float], ...]] = ((-0.5, 0.125, 0.3), (0.75, 0.0, 0.3))
    ring_geom_name: str = "ring"

    rod_name: str = "rod"
    rod_body_name: str = "rod_body"
    rod_position: npt.NDArray = np.array([0.0, 0.175, 0.25])
    rod_rotation: npt.NDArray = euler_to_quat([np.pi * 0.5, 0.0, 0.0], order="xyz")
    rod_target_top_site_name: str = "rod_target_top_site"
    rod_target_bottom_site_name: str = "rod_target_bottom_site"

    ring_insertion_height_threshold: float = 0.05
    ring_insertion_radius_threshold: float = 0.05
    initial_lift: bool = True

    # robot
    home_arm_qpos: npt.NDArray = np.array([-1.06245998, 3.70099487, 1.59522048, -0.33521059, 1.21703438, -1.39767057])
    home_gripper_qpos: npt.NDArray = np.array([0.1, 0.0, 0.1, 0.0, 0.1, 0.0])
...
class RingHangingRobotEnvBuilder(RobotEnvBuilder):
    _config: RingHangingRobotEnvBuilderConfig

    def __init__(self, config: RingHangingRobotEnvBuilderConfig):
    ...
    def build_env_model(
        ...
        ring = Ring(
            base_body_name=self._config.ring_body_name,
            ring_joint_names=[self._config.ring_joint_name],
            ring_body_names=[self._config.ring_body_name],
            geom_root_name=self._config.ring_name,
            ring_site_name=self._config.ring_site_name,
            ring_handle_site_name=self._config.ring_handle_site_name,
        )
        rod = Rod(
            base_body_name=self._config.rod_body_name,
            rod_joint_names=[],
            rod_body_names=[self._config.rod_body_name],
            geom_root_name=self._config.rod_name,
            rod_top_site_name=self._config.rod_target_top_site_name,
            rod_bottom_site_name=self._config.rod_target_bottom_site_name,
            insertion_height_threshold=self._config.ring_insertion_height_threshold,
            insertion_radius_threshold=self._config.ring_insertion_radius_threshold,
        )
    ...
    def _build_objects_xml(self, generator: MJCFGenerator):
        info = add_ring(
            ...
            ring_name=self._config.ring_name,
            ring_rotation=self._config.ring_rotation,
            ring_position=self._config.ring_position,
            ring_handle_site_name=self._config.ring_handle_site_name,
            ring_body_name=self._config.ring_body_name,
            ring_joint_name=self._config.ring_joint_name,
            ring_site_name=self._config.ring_site_name,
        )
        ...
        add_rod(
            generator=generator,
            asset_dir_path=self._config.asset_dir_path,
            rod_name=self._config.rod_name,
            rod_rotation=self._config.rod_rotation,
            rod_position=self._config.rod_position,
            rod_body_name=self._config.rod_body_name,
            rod_target_top_site_name=self._config.rod_target_top_site_name,
            rod_target_bottom_site_name=self._config.rod_target_bottom_site_name,
        )
    ...
    def _build_env(
        ...
        env: MujocoEnv = gymnasium.make(  # type: ignore
            ...
            segmentation_object_names=(
                ...
                self._config.rod_name,
                self._config.ring_name,
                "task_area_marker_1",
                "task_area_marker_2",
                "task_area_marker_3",
                "task_area_marker_4",
