...
from mujaco_gym_lite.environment_tools.mujoco_env.xmls.metaworld_objects.windows.side import add_side_window
...
@dataclass
class WindowRobotEnvBuilderConfig(RobotEnvBuilderConfig):
    # model settings
    window_name: str = "window"
    window_joint_name: str = "window_joint"
    window_body_name: str = "window_body"
    window_position: npt.NDArray = np.array([0.1, 0.3, 0.3])
    window_rotation: npt.NDArray = euler_to_quat(0.0, order="z")
    window_name: str = "window"
    window_handle_name: str = "window_handle"
    window_handle_site_name: str = "window_handle_site"
    obstacle_positions: Optional[tuple[tuple[float, float, float], ...]] = ((-0.5, 0.125, 0.3), (0.75, 0.0, 0.3))
    window_joint_range: tuple[float, float] = (0.0, 0.25)
    window_geom_name: str = "window"

    # robot
    home_arm_qpos: npt.NDArray = np.array([-2.26898474, 3.61681055, 1.13287953, -4.31113752, 1.27554416, 0.10598583])
...
class WindowRobotEnvBuilder(RobotEnvBuilder):
    _config: WindowRobotEnvBuilderConfig

    def __init__(self, config: WindowRobotEnvBuilderConfig):
    ...
    def build_env_model(
        ...
        window = Window(
            self._config.window_body_name,
            [self._config.window_joint_name],
            [self._config.window_body_name],
            self._config.window_name,
            window_joint_range=self._config.window_joint_range,
            window_handle_geom_name=self._config.window_handle_name,
            window_handle_site_name=self._config.window_handle_site_name,
        )
    ...
    def _build_objects_xml(self, generator: MJCFGenerator):
        window_info = add_side_window(
            generator=generator,
            asset_dir_path=self._config.asset_dir_path,
            window_name=self._config.window_name,
            window_rotation=self._config.window_rotation,
            window_position=self._config.window_position,
            window_handle_site_name=self._config.window_handle_site_name,
            window_handle_name=self._config.window_handle_name,
            window_body_name=self._config.window_body_name,
            window_joint_name=self._config.window_joint_name,
        )
        ...
        if self._config.add_task_area_marker:
            add_workspace_marker(
                generator,
                "task_area_marker_1",
                marker_positions=[np.array([0.0, -0.08, 0.1])],
                rgbas=[np.array([0.0, 0.5, 0.0, 1.0]) for _ in range(1)],
                size=np.array([0.015, 0.015, 0.015]),
                attach_body=window_info["window_handle"]["body_name"],
            )
            add_workspace_marker(
                generator,
                "task_area_marker_2",
            ...
            add_workspace_marker(
                generator,
                "task_area_marker_3",
                marker_positions=[np.array([0.4, 0.275, 0.5])],
                rgbas=[np.array([0.0, 0.5, 0.0, 1.0]) for _ in range(1)],
            ...
            add_workspace_marker(
                generator,
                "task_area_marker_4",
                marker_positions=[np.array([-0.2, 0.275, 0.5])],
                rgbas=[np.array([0.0, 0.5, 0.0, 1.0]) for _ in range(1)],
                size=np.array([0.025, 0.025, 0.025]),
                attach_body="worldbody",
            )
            add_workspace_marker(
                generator,
                "task_area_marker_5",
                marker_positions=[np.array([0.4, 0.275, 0.1])],
                rgbas=[np.array([0.0, 0.5, 0.0, 1.0]) for _ in range(1)],
                size=np.array([0.025, 0.025, 0.025]),
                attach_body="worldbody",
            )
            add_workspace_marker(
                generator,
                "task_area_marker_6",
                marker_positions=[np.array([-0.2, 0.275, 0.1])],
                rgbas=[np.array([0.0, 0.5, 0.0, 1.0]) for _ in range(1)],
                size=np.array([0.025, 0.025, 0.025]),
                attach_body="worldbody",
            )
    ...
    def _build_env(
        ...
        env: MujocoEnv = gymnasium.make(  # type: ignore
            ...
            segmentation_object_names=(
                ...
                self._config.window_handle_name,
                "task_area_marker_1",
                "task_area_marker_2",
                "task_area_marker_3",
                "task_area_marker_4",
