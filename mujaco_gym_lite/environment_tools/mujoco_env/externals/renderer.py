...
class WindowViewer(BaseRender):
    ...
    def render(self, camera_id=None):
        ...
        Renders the environment geometries in the OpenGL glfw window:
            ...
            2. Update the geometries used for rendering based on the current state of the model -
                `mujoco.mjv_updateScene()`.
            3. Add markers to scene, these are additional geometries to include in the model, i.e arrows,
            https://mujoco.readthedocs.io/en/latest/APIreference.html?highlight=arrow#mjtgeom.
    ...
    def _cursor_pos_callback(self, window: "glfw.LP__GLFWwindow", xpos: float, ypos: float):
        ...
        mujoco.mjv_moveCamera(self.model, action, dx / width, dy / height, self.scn, self.cam)
...
class OffScreenViewer(BaseRender):
    ...
    def render(
        ...
        if render_mode in ["rgb_array", "rgbd_tuple"]:
            ...
            if segmentation:
                # UPDATE: Need cast from numpy 2.0
                rgb_int = rgb_img.astype(np.int32)
                seg_img = rgb_int[:, :, 0] + rgb_int[:, :, 1] * (2**8) + rgb_int[:, :, 2] * (2**16)
