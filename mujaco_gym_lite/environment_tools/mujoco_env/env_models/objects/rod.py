...
class Rod(EnvModel):
    def __init__(
        self,
        base_body_name: str,
        rod_joint_names: list[str],
        rod_body_names: list[str],
        geom_root_name: str,
        ...
        super().__init__(base_body_name, rod_joint_names, rod_body_names, geom_root_name)
        self._rod_bottom_site_name = rod_bottom_site_name
        self._rod_top_site_name = rod_top_site_name
        self._insertion_height_threshold = insertion_height_threshold  # y axis
        self._insertion_radius_threshold = insertion_radius_threshold  # xz plane
    ...
    def is_ring_inserting(self, ring: AssemblyRing) -> tuple[bool, dict[str, npt.NDArray | bool]]:
        ...
        # y-axis (height) and xz-plane (radius) checks
        is_inserted_height = (target_rod_position[1] - ring_position[1]) < self._insertion_height_threshold

        # radius check in xz-plane
        is_inserted_radius = bool(
            np.linalg.norm(target_rod_position[[0, 2]] - ring_position[[0, 2]]) < self._insertion_radius_threshold
        )
