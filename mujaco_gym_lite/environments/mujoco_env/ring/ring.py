...
from mujaco_gym_lite.environment_tools.mujoco_env.env_models.objects.ring import Ring
from mujaco_gym_lite.environment_tools.mujoco_env.env_models.objects.rod import Rod
...
def _object_obervation(
    ...
    ring_pos = np.array(ring.ring_position(), dtype=np.float32).reshape((3,))
    ...
    object_observation = {
        "ring/handle/position": np.array(ring.ring_handle_position(), dtype=np.float32),
        "ring/ring/position": np.array(ring.ring_position(), dtype=np.float32),
        "ring/ring/qpos": clipped_qpos,
        "rod/bottom/position": np.array(rod.rod_bottom_position(), dtype=np.float32),
        "rod/top/position": np.array(rod.rod_top_position(), dtype=np.float32),
    }
    gripper_hold_handle, _ = ring.has_handle_touch(robot.geom_root_name())
    return object_observation, {"ring/holded_handle_by_gripper": gripper_hold_handle}
...
def _compute_reward_ring(
    ...
    target_rod_bottom_position: npt.NDArray[Any],
) -> tuple[float, dict]:

    pos_error = target_rod_top_position - ring_center
    init_pos_error = target_rod_top_position - init_ring_center
    # xz plane
    radius = np.linalg.norm(pos_error[[0, 2]])

    height = abs(pos_error[1] - 0.05)
    init_radius = np.linalg.norm(init_pos_error[[0, 2]])

    reward_for_xz_alignment = tolerance(
        radius,
        bounds=(0, 0.025),
        margin=np.abs(init_radius),
        sigmoid="long_tail",
    )
    reward_for_y_alignment = tolerance(
        height,
        bounds=(0, 0.01),
        margin=0.3,
        sigmoid="long_tail",
    ...
    if radius < 0.0525:
        reward_for_y_alignment = 1.0
        reward_for_height = tolerance(
            abs(max(float(target_rod_bottom_position[1] - ring_center[1] + 0.015), 0.0)),
            bounds=(0, 0.01),
            margin=0.4,
            sigmoid="long_tail",
        )
    ...
    reward = 3.0 * reward_for_height + 1.0 * reward_for_xz_alignment + 1.0 * reward_for_y_alignment
    return reward, {"reward/xz_error": radius}
...
def _step_to_target(robot, model, data, frame_skip, target_pos, target_quat, max_trans_step=0.05, render_fn=None):
    for _ in range(100):
        ...
        robot.apply_gripper_control(np.array([200.0, 220.0], dtype=np.float32))
        robot_action(
            robot,
            ...
            open=False,
            home=False,
            gripper_command=np.array([200.0, 220.0], dtype=np.float32),
        )
...
def _sample_donut(center_xz=(0.0, 0.0), r_inner=0.05, r_outer=0.075, y_range=(-0.15, -0.1)):
...
class RingHangingRobotEnv(MujocoEnv):
    metadata = {
    ...
    def __init__(
        ...
        self._ring.load_mj_model_and_data(self.model, self.data)
        self._rod.load_mj_model_and_data(self.model, self.data)
    ...
    def step(self, action: Any):
        ...
        robot_action(
            ...
            gripper_command=np.array([200.0, 220.0], dtype=np.float32),  # always close
        )
        ...
        ob, ob_info = _env_observation(
            ...
            ring_position_min=self.unwrapped.observation_space["ring/ring/qpos"].low,
            ring_position_max=self.unwrapped.observation_space["ring/ring/qpos"].high,
            ring=self._ring,
            rod=self._rod,
            robot=self._robot,
        ...
        if self._mocap_coordinate_marker is not None:
            self._mocap_coordinate_marker.move(
                create_transformation_matrix(translation=ob["rod/top/position"] + np.array([0.0, 0.075, 0.0]))
            )

        reward, reward_info = _compute_reward_ring(
            ring_center=ob["ring/ring/position"],
            target_rod_top_position=ob["rod/top/position"],
            target_rod_bottom_position=ob["rod/bottom/position"],
            init_ring_center=self._initial_ring_center_position,
        )
        is_ring_inserting, info = self._rod.is_ring_inserting(ring=self._ring)
        ob_info["task/success"] = is_ring_inserting
        if is_ring_inserting:
            reward += 5.0  # additional bonus if success

        if not ob_info["ring/holded_handle_by_gripper"]:
    ...
    def reset(self, *, seed: Optional[int] = None, return_info: bool = False, options: Optional[dict] = None):
        ...
        # apply close gripper
        for _ in range(50):
            self._robot.apply_gripper_control(np.array([0.0, 0.0], dtype=np.float32))
            angles = self._robot.solve_ik(
                create_transformation_matrix(
                    translation=np.array([0.275, -0.13, 0.115]),
                    rotation=self._target_initial_end_effector_rotation,
                    rotation_type="quaternion",
                ),
            )
            assert angles is not None
            self._robot.apply_arm_control(angles)
            mujoco.mj_step(self.model, self.data, nstep=self.frame_skip)

        for _ in range(50):
            self._robot.apply_gripper_control(np.array([200.0, 220.0], dtype=np.float32))
            mujoco.mj_step(self.model, self.data, nstep=self.frame_skip)

        if self._initial_lift:
            ee_pose = self._robot.end_effector_pose()
            rod_top_position = np.array(self._rod.rod_top_position(), dtype=np.float32)
            rod_ring_position = np.array(self._ring.ring_position(), dtype=np.float32)
            curr_pos = extract_position(ee_pose)
            difference_position = rod_ring_position - curr_pos

            target_pos = _sample_donut(center_xz=tuple(rod_top_position[[0, 2]].tolist()), r_inner=0.075, r_outer=0.1)
            target_pos[[0, 2]] -= difference_position[[0, 2]]

            curr_quat = extract_rotation(ee_pose, rotation_type="quaternion")
            target_quat = curr_quat

            xz_target_pos = np.array([target_pos[0], curr_pos[1], target_pos[2]])
            _step_to_target(self._robot, self.model, self.data, self.frame_skip, xz_target_pos, target_quat)
            y_target_pos = np.array([target_pos[0], target_pos[1], target_pos[2]])
            _step_to_target(self._robot, self.model, self.data, self.frame_skip, y_target_pos, target_quat)
        ...
        self._initial_ring_center_position = ob["ring/ring/position"]
        return ob, {}
    ...
    def _build_observation_space(
        ...
        object_observation_space = {
            "ring/handle/position": gymnasium.spaces.Box(-np.inf, np.inf, shape=(3,), dtype=np.float32),
            "ring/ring/position": gymnasium.spaces.Box(-np.inf, np.inf, shape=(3,), dtype=np.float32),
            "ring/ring/qpos": gymnasium.spaces.Box(
            ...
            "rod/bottom/position": gymnasium.spaces.Box(-np.inf, np.inf, shape=(3,), dtype=np.float32),
            "rod/top/position": gymnasium.spaces.Box(-np.inf, np.inf, shape=(3,), dtype=np.float32),
        }
        observation_space = {}
