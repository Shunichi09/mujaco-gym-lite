...
def _object_obervation(
    lid: Lid, position_bounds: list[npt.NDArray]
) -> tuple[dict[str, npt.NDArray], dict[str, list[npt.NDArray]]]:
    lid_position = lid.handle_low_center_position()
    orig_position = lid_position.copy()
    ...
    if not np.allclose(orig_position, clipped_position, atol=1e-6):
        logger.debug(f"Lid position clipped:\n  before={orig_position}\n  after ={clipped_position}")

    object_observation = {
        "lid/position": np.array(clipped_position, dtype=np.float32),
    }
...
def _env_observation(
    ...
    lid_position_min: npt.NDArray,
    lid_position_max: npt.NDArray,
    # object obs settings
    lid: Lid,
    open_top_box: OpenTopBox,
    robot: Robot,
    ...
    object_obs, object_info = _object_obervation(lid=lid, position_bounds=[lid_position_min, lid_position_max])
    ...
    distance_to_target = np.linalg.norm(lid.handle_low_center_position() - open_top_box.open_top_center_position())

    robot_special_obs = {
        "robot/distance_to_lid": np.array([distance_to_target], dtype=np.float32),
    }
...
def _compute_reward(
    ...
    target_lid_position: npt.NDArray,
    initial_lid_position: npt.NDArray,
):
    lid_error = float(np.linalg.norm(lid_position - target_lid_position))
    reward_for_lid = tolerance(
        lid_error,
        bounds=(0, 0.01),
        margin=0.5,
        sigmoid="long_tail",
    )
    return float(reward_for_lid), {"reward/reward_for_lid": reward_for_lid}
...
def _step_to_target(
    ...
    target_quat,
    max_trans_step=0.05,
    gripper_target=np.array([250.0, 250.0]),
    render_fn=None,
):
    for _ in range(50):
        ...
        robot.apply_gripper_control(gripper_target)
        robot_action(
            robot,
...
class LiddedBoxRobotEnv(MujocoEnv):
    metadata = {
    ...
    def __init__(
        ...
        self._robot = robot
        self._open_top_box = open_top_box
        self._lid = lid
        ...
        self._open_top_box.load_mj_model_and_data(self.model, self.data)
        self._lid.load_mj_model_and_data(self.model, self.data)
    ...
    def step(self, action: Any):
        ...
        robot_action(
            ...
            gripper_command=np.array([250.0, 250.0]),
            open=False,
            close=False,
        ...
        ob, ob_info = _env_observation(
            ...
            lid_position_min=self.observation_space["lid/position"].low,
            lid_position_max=self.observation_space["lid/position"].high,
            lid=self._lid,
            open_top_box=self._open_top_box,
            robot=self._robot,
        ...
        reward, reward_info = _compute_reward(
            lid_position=self._lid.handle_low_center_position(),
            target_lid_position=self._open_top_box.open_top_center_position(),
            initial_lid_position=self._initial_lid_position,
        )
        ob_info.update(reward_info)
        ob_info["task/success"] = self._open_top_box.is_covered(self._lid)
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        if self._initial_lift:
            ...
            lid_handle_position = self._lid.handle_center_position().copy()
            lid_handle_position[2] = 0.08
            ...
            _step_to_target(
                ...
                self.data,
                self.frame_skip,
                target_pos=lid_handle_position,
                target_quat=quat,
                gripper_target=np.array([110.0, 110.0]),
                max_trans_step=0.05,
            )
            _step_to_target(
                self._robot,
        ...
        if self._initial_position_noise:
            assert self._initial_lift
            # apply initial position noise
            initial_noise = rand_min_max(min_val=np.array([-0.45, -0.1, 0.05]), max_val=np.array([0.0, 0.15, 0.15]))
            _step_to_target(
                self._robot,
                self.model,
                self.data,
                self.frame_skip,
                target_pos=lid_handle_position + initial_noise,
                target_quat=quat,
                gripper_target=np.array([250.0, 250.0]),
                max_trans_step=0.05,
            )
        ...
        self._initial_lid_position = ob["lid/position"]
        self._initial_end_effector_position = ob["robot/end_effector/position"]

        if self._mocap_coordinate_marker is not None:
            self._mocap_coordinate_marker.move(create_transformation_matrix(translation=self._initial_lid_position))
        return ob, ob_info
    ...
    def _build_observation_space(
        ...
        object_observation_space = {
            "lid/position": gymnasium.spaces.Box(
                low=self._lid._lid_min_position,
                high=self._lid._lid_max_position,
                shape=(3,),
                dtype=np.float32,
            ),
        }
        ...
        observation_space.update(
            {
                "robot/distance_to_lid": gymnasium.spaces.Box(0.0, np.inf, shape=(1,), dtype=np.float32),
            }
        )
