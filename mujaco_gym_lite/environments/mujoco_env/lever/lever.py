...
from mujaco_gym_lite.environment_tools.mujoco_env.env_models.objects.lever import Lever
...
def _object_obervation(lever: Lever, qpos_bounds) -> tuple[dict[str, npt.NDArray], dict[str, list[npt.NDArray]]]:
    lever_qpos = np.array(lever.joint_qpos(), dtype=np.float32).reshape((1,))
    orig_qpos = lever_qpos.copy()
    ...
    if not np.array_equal(orig_qpos, clipped_qpos):
        logger.debug(f"Lever position clipped:\n  before={orig_qpos}\n  after ={clipped_qpos}")

    object_observation = {
        "lever/qpos": clipped_qpos,
        "lever/handle_position": np.array(lever.handle_center_position(), dtype=np.float32),
    }
...
def _env_observation(
    ...
    lever_qpos_min: npt.NDArray,
    lever_qpos_max: npt.NDArray,
    # object obs settings
    lever: Lever,
    robot: Robot,
    ...
    object_obs, object_info = _object_obervation(lever=lever, qpos_bounds=[lever_qpos_min, lever_qpos_max])
    ...
    robot_special_obs = {
        "robot/distance_to_handle_center": np.array(
            [lever.distance_to_handle_center(extract_position(robot.end_effector_pose()))], dtype=np.float32
        ),
    }
...
def _compute_reward(
    *,
    ...
    """compute reward of lever pulling task"""
    ...
    reward_for_opening = tolerance(
        handle_error,
        bounds=(0, 0.01),
        margin=np.deg2rad(60.0),
        sigmoid="long_tail",
    )

    target_position = handle_position - np.array([0.0, 0.03, 0.03])
    gripper_error = (target_position - end_effector_position) * np.array([3.0, 1.0, 1.0])

    reward_for_caging = tolerance(
        float(np.linalg.norm(gripper_error)),
        bounds=(0.0, 0.045),  # target range
        margin=0.25,  # margin for decays
        sigmoid="long_tail",  # decay function shape
    )
    reward = 2.0 * reward_for_caging + 5.0 * reward_for_opening
    return float(reward), {
...
class LeverRobotEnv(MujocoEnv):
    ...
    def __init__(
        ...
        self._lever.load_mj_model_and_data(self.model, self.data)
    ...
    def step(self, action: Any):
        ...
        ob, ob_info = _env_observation(
            ...
            lever_qpos_min=self.observation_space["lever/qpos"].low,
            lever_qpos_max=self.observation_space["lever/qpos"].high,
            lever=self._lever,
            robot=self._robot,
        ...
        reward, reward_info = _compute_reward(
            handle_qpos=ob["lever/qpos"][0],
            target_handle_qpos=np.deg2rad(60.0),
            handle_position=ob["lever/handle_position"],
        ...
        ob_info["task/success"] = self._lever.is_down(target_angle=np.deg2rad(55.0))
        if ob_info["task/success"]:
            reward += 3.0

        if self._mocap_coordinate_marker is not None:
            self._mocap_coordinate_marker.move(create_transformation_matrix(translation=ob["lever/handle_position"]))
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        if self._initial_position_noise:
            ...
            # lever: np.array([0.1, -0.08, 0.125], dtype=np.float32)
            ee_pose = self._robot.end_effector_pose()
            lever_handle_pos = self._lever.handle_center_position().copy()
            initial_noise = rand_min_max(min_val=np.array([0.0, -0.05, -0.025]), max_val=np.array([0.15, 0.05, 0.025]))
            pos = lever_handle_pos + initial_noise
        ...
        self._initial_handle_position = ob["lever/handle_position"]
    ...
    def observation_info_keys(self) -> tuple[str, ...]:
        ...
        keys.append("lever/contact_positions/handle")
        keys.append("lever/contact_names/handle")
    ...
    def _build_observation_space(
        ...
        assert self._lever._lever_joint_range[0] < self._lever._lever_joint_range[1]
        object_observation_space = {
            "lever/qpos": gymnasium.spaces.Box(
                low=self._lever._lever_joint_range[0],
                high=self._lever._lever_joint_range[1],
                shape=(1,),
                dtype=np.float32,
            ),
            "lever/handle_position": gymnasium.spaces.Box(-np.inf, np.inf, shape=(3,), dtype=np.float32),
        }
