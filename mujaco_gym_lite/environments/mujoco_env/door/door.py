...
def _object_obervation(door: SlidingDoor, qpos_bounds) -> tuple[dict[str, npt.NDArray], dict[str, list[npt.NDArray]]]:
    door_qpos = np.array(door.joint_qpos(), dtype=np.float32).reshape((1,))
    orig_qpos = door_qpos.copy()
    ...
    if not np.array_equal(orig_qpos, clipped_qpos):
        logger.debug(f"Door position clipped:\n  before={orig_qpos}\n  after ={clipped_qpos}")

    object_observation = {
        "door/qpos": clipped_qpos,
        "door/handle_position": np.array(door.handle_center_position(), dtype=np.float32)
        + np.array([*qpos_bounds[1], 0.0, 0.0], dtype=np.float32),
    }
...
def _env_observation(
    ...
    door_qpos_min: npt.NDArray,
    door_qpos_max: npt.NDArray,
    # object obs settings
    door: SlidingDoor,
    robot: Robot,
    ...
    object_obs, object_info = _object_obervation(door=door, qpos_bounds=[door_qpos_min, door_qpos_max])
    ...
    touch_handle, touch_handle_info = door.has_handle_touch(robot.geom_root_name())
    robot_special_obs = {
        "robot/distance_to_handle_center": np.array(
            [door.distance_to_handle_center(extract_position(robot.end_effector_pose()))], dtype=np.float32
        ),
...
def _compute_reward(
    ...
    door_open_threshold: float,
    door_max_joint_qpos: float,
):
    """compute reward of door opening task"""
    handle_error = float(np.linalg.norm(handle_position - target_handle_position))
    reward_for_opening = tolerance(
        handle_error,
        bounds=(0, max(0.0, door_open_threshold - 0.02)),
        margin=np.abs(door_max_joint_qpos),
        sigmoid="long_tail",
    )

    additional = np.array([0.025, 0.0, 0.0])
    gripper_error = handle_position - end_effector_position - additional
    gripper_error_init = initial_handle_position - initial_end_effector_position - additional
    ...
    reward = 3.0 * reward_for_inplace + 6.0 * reward_for_opening
...
def _step_to_target(robot, model, data, frame_skip, target_pos, target_quat, max_trans_step=0.05, render_fn=None):
    for _ in range(100):
        ...
        robot.apply_gripper_control(np.array([150.0, 150.0], dtype=np.float32))
        robot_action(
            robot,
            ...
            open=False,
            home=False,
            gripper_command=np.array([150.0, 150.0], dtype=np.float32),
        )
...
class SlidingDoorRobotEnv(MujocoEnv):
    metadata = {
    ...
    def __init__(
        ...
        self._door.load_mj_model_and_data(self.model, self.data)
    ...
    def step(self, action: Any):
        ...
        robot_action(
            ...
            gripper_command=np.array([150.0, 150.0]),
            open=False,
            close=False,
        ...
        ob, ob_info = _env_observation(
            ...
            door_qpos_min=self.observation_space["door/qpos"].low,
            door_qpos_max=self.observation_space["door/qpos"].high,
            door=self._door,
            robot=self._robot,
        ...
        reward, reward_info = _compute_reward(
            handle_position=ob["door/handle_position"],
            end_effector_position=ob["robot/end_effector/position"],
            target_handle_position=self._initial_handle_position
            - np.array([self._door._door_joint_range[1], 0.0, 0.0]),  # NOTE: Check negative or positive
            initial_end_effector_position=self._initial_end_effector_position,
            initial_handle_position=self._initial_handle_position,
            door_open_threshold=self._door._open_and_close_thredshold,
            door_max_joint_qpos=self._door._door_joint_range[1],
        )
        ob_info["task/success"] = self._door.is_open()
        if ob_info["task/success"]:
            reward += 3.0
        ob_info.update(reward_info)
        if self._mocap_coordinate_marker is not None:
            self._mocap_coordinate_marker.move(create_transformation_matrix(translation=ob["door/handle_position"]))
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        if self._initial_position_noise:
            # initial robot position: np.array([0.15, 0.0, 0.45], dtype=np.float32)
            # door: np.array([0.1, 0.135, 0.3], dtype=np.float32)
            ee_pose = self._robot.end_effector_pose()
            initial_noise = rand_min_max(
                min_val=np.array([-0.225, -0.225, -0.1]), max_val=np.array([0.225, -0.15, 0.1])
            )
            pos = (
                self._door.handle_center_position()
                + initial_noise
                + np.array([self._door._door_joint_range[1], 0.0, 0.0])
            )
        ...
        self._initial_handle_position = ob["door/handle_position"]
    ...
    def observation_info_keys(self) -> tuple[str, ...]:
        ...
        keys.append("door/contact_positions/handle")
        keys.append("door/contact_names/handle")
    ...
    def _build_observation_space(
        ...
        assert self._door._door_joint_range[0] < self._door._door_joint_range[1]
        object_observation_space = {
            "door/qpos": gymnasium.spaces.Box(
                low=self._door._door_joint_range[0], high=self._door._door_joint_range[1], shape=(1,), dtype=np.float32
            ),
            "door/handle_position": gymnasium.spaces.Box(-np.inf, np.inf, shape=(3,), dtype=np.float32),
        }
