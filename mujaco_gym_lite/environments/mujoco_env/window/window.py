...
from mujaco_gym_lite.environment_tools.mujoco_env.env_models.objects.window import Window
...
from mujaco_gym_lite.utils.transforms import create_transformation_matrix, extract_position, matrix_to_quat
from mujaco_gym_lite.utils.views import view_to_rotation


def _object_obervation(window: Window) -> tuple[dict[str, npt.NDArray], dict[str, list[npt.NDArray]]]:
    object_observation = {
        "window/qpos": np.array(window.joint_qpos(), dtype=np.float32).reshape((1,)),
        "window/handle_position": np.array(window.handle_center_position(), dtype=np.float32),
    }
...
def _env_observation(
    ...
    object_obs, object_info = _object_obervation(window=window)
    ...
    robot_obs, robot_observation_info = robot_observation(
        robot=robot, target_initial_end_effector_rotation=target_initial_end_effector_rotation
    )
    env_obs = {
    ...
    touch_handle, touch_handle_info = window.has_handle_touch(robot.geom_root_name())
    robot_special_obs = {
        "robot/distance_to_handle_center": np.array(
            [window.distance_to_handle_center(extract_position(robot.end_effector_pose()))], dtype=np.float32
        ),
...
def _compute_reward(
    ...
    window_open_threshold: float,
    window_max_joint_qpos: float,
):
    """compute reward of window opening task

    Reference:
        https://github.com/Farama-Foundation/Metaworld/blob/cca35cff0ec62f1a18b11440de6b09e2d10a1380/metaworld/envs/mujoco/sawyer_xyz/v2/sawyer_window_open_v2.py#L109C1-L151C10
    """
    handle_error = float(np.linalg.norm(handle_position - target_handle_position))
    reward_for_opening = tolerance(
        handle_error,
        bounds=(0, max(0.0, window_open_threshold - 0.02)),
        margin=np.abs(window_max_joint_qpos),
        sigmoid="long_tail",
    )

    gripper_error = (handle_position + np.array([-0.0425, 0.0, 0.0])) - end_effector_position
    gripper_error_init = initial_handle_position - initial_end_effector_position

    # NOTE:
    # This function computes a tolerance score that indicates how close an input value (or array of values) is
...
def _step_to_target(robot, model, data, frame_skip, target_pos, target_quat, max_trans_step=0.05, render_fn=None):
    for _ in range(100):
        ...
        robot.apply_gripper_control(np.array([200.0, 200.0]))
        robot_action(
            robot,
            ...
            open=False,
            home=False,
            gripper_command=np.array([200.0, 200.0]),  # always close
        )
...
class WindowRobotEnv(MujocoEnv):
    metadata = {
    ...
    def __init__(
        ...
        self._window.load_mj_model_and_data(self.model, self.data)
    ...
    def workspace() -> tuple[npt.NDArray, npt.NDArray]:
        return np.array([-0.4, -0.15, 0.125]), np.array([0.4, 0.2, 0.5])
    ...
    def step(self, action: Any):
        ...
        reward, reward_info = _compute_reward(
            handle_position=ob["window/handle_position"],
            end_effector_position=ob["robot/end_effector/position"],
            target_handle_position=self._initial_handle_position
            + np.array([self._window._window_joint_range[1], 0.0, 0.0]),  # NOTE: Check negative or positive
            initial_end_effector_position=self._initial_end_effector_position,
            initial_handle_position=self._initial_handle_position,
            window_open_threshold=self._window._open_and_close_thredshold,
            window_max_joint_qpos=self._window._window_joint_range[1],
        )
        # index = min(self._current_timesteps() - 1, len(TARGET_TRAJECTORY["robot/end_effector/position"]) - 1)
        # reward, reward_info = _compute_distance_reward(
        #    robot_position=ob["robot/end_effector/position"],
        #    handle_position=ob["window/handle_position"],
        #    target_robot_position=np.array(TARGET_TRAJECTORY["robot/end_effector/position"][index]),
        #    target_handle_position=np.array(TARGET_TRAJECTORY["window/handle_position"][index]),
        # )
        ob_info["task/success"] = self._window.is_open()
        ob_info.update(reward_info)
        if self._mocap_coordinate_marker is not None:
            self._mocap_coordinate_marker.move(
                create_transformation_matrix(translation=ob["window/handle_position"] + np.array([-0.0425, 0.0, 0.0]))
            )
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        self._initial_handle_position = ob["window/handle_position"]
        self._initial_end_effector_position = ob["robot/end_effector/position"]

        if self._mocap_coordinate_marker is not None:
            self._mocap_coordinate_marker.move(create_transformation_matrix(translation=self._initial_handle_position))
        return ob, ob_info
    ...
    def observation_info_keys(self) -> tuple[str, ...]:
        ...
        keys.append("window/contact_positions/handle")
        keys.append("window/contact_names/handle")
    ...
    def _build_observation_space(
        ...
        object_observation_space = {
            "window/qpos": gymnasium.spaces.Box(-np.inf, np.inf, shape=(1,), dtype=np.float32),
            "window/handle_position": gymnasium.spaces.Box(-np.inf, np.inf, shape=(3,), dtype=np.float32),
        }
        ...
        observation_space.update(self._build_robot_observation_space(robot=self._robot))
        observation_space.update(self._build_env_observation_space())

        # add special
        observation_space.update(
            {
                "robot/distance_to_handle_center": gymnasium.spaces.Box(-np.inf, np.inf, shape=(1,), dtype=np.float32),
    ...
    def _build_action_space(self, mocap_cameras: tuple[MocapCamera, ...]):
        robot_action_space = {
            "robot/end_effector/position": gymnasium.spaces.Box(-np.inf, np.inf, shape=(3,), dtype=np.float32),
