...
def _object_obervation(
    soccer_ball: SoccerBall, position_bounds: list[npt.NDArray]
) -> tuple[dict[str, npt.NDArray], dict[str, list[npt.NDArray]]]:
    soccer_ball_position = extract_position(soccer_ball.body_pose()[0])
    orig_position = soccer_ball_position.copy()
    clipped_position = np.clip(orig_position, a_min=position_bounds[0], a_max=position_bounds[1], dtype=np.float32)
    if not np.allclose(orig_position, clipped_position, atol=1e-6):
        logger.debug(f"Soccer ball position clipped:\n  before={orig_position}\n  after ={clipped_position}")

    object_observation = {
        "soccer_ball/position": np.array(clipped_position, dtype=np.float32),
    }
...
def _env_observation(
    ...
    soccer_ball_position_min: npt.NDArray,
    soccer_ball_position_max: npt.NDArray,
    # object obs settings
    ...
    object_obs, object_info = _object_obervation(
        soccer_ball=soccer_ball, position_bounds=[soccer_ball_position_min, soccer_ball_position_max]
    )
    ...
    distance_to_ball = np.linalg.norm(
        extract_position(robot.end_effector_pose()) - extract_position(soccer_ball.body_pose()[0])
    )
    robot_special_obs = {
        "robot/distance_to_ball": np.array([distance_to_ball], dtype=np.float32),
    }
...
def _compute_reward(
    ...
    target_goal_position: npt.NDArray,
    initial_ball_position: npt.NDArray,
    initial_end_effector_position: npt.NDArray,
):
    target_ball = target_goal_position[:2] - np.array([0.0, 0.055])
    ball_error = float(np.linalg.norm((ball_position[:2] - target_ball) * np.array([1.0, 1.0])))
    reward_for_ball = tolerance(
        ball_error,
        bounds=(0, 0.05),
        margin=0.4,
        sigmoid="long_tail",
    ...
    ball_position_3d = np.array([ball_position[0], ball_position[1] - 0.09, 0.05])
    gripper_error = (ball_position_3d - end_effector_position) * np.array([1.0, 1.0, 1.0])
    reward_for_caging = tolerance(
        float(np.linalg.norm(gripper_error)),
        bounds=(0.0, 0.05),  # target range
        margin=0.4,  # margin for decays
        sigmoid="long_tail",  # decay function shape
    )

    if end_effector_position[2] < 0.075:
        reward = 5.0 * reward_for_caging + 2.0 * reward_for_ball
    else:
        reward = 1.0 * reward_for_caging + 0.0 * reward_for_ball

    return float(reward), {
        "reward/reward_for_caging": reward_for_caging,
        "reward/reward_for_ball": reward_for_ball,
    }
...
def _step_to_target(robot, model, data, frame_skip, target_pos, target_quat, max_trans_step=0.05, render_fn=None):
    for _ in range(100):
        ...
        robot.apply_gripper_control(np.array([110.0, 110.0]))
        robot_action(
            robot,
            ...
            open=False,
            home=False,
            gripper_command=np.array([110.0, 110.0]),
        )
...
class SoccerRobotEnv(MujocoEnv):
    metadata = {
    ...
    def __init__(
        ...
        self._robot = robot
        self._soccer_ball = soccer_ball
        self._soccer_goal = soccer_goal
        ...
        self._soccer_ball.load_mj_model_and_data(self.model, self.data)
        self._soccer_goal.load_mj_model_and_data(self.model, self.data)
    ...
    def step(self, action: Any):
        ...
        ob, ob_info = _env_observation(
            ...
            soccer_ball_position_min=self.observation_space["soccer_ball/position"].low,
            soccer_ball_position_max=self.observation_space["soccer_ball/position"].high,
            soccer_ball=self._soccer_ball,
            robot=self._robot,
        ...
        reward, reward_info = _compute_reward(
            ...
            ball_position=ob["soccer_ball/position"],
            initial_ball_position=self._initial_ball_position,
            target_goal_position=self._soccer_goal.center_goal_position(),
        )
        ob_info.update(reward_info)
        ob_info["task/success"] = self._soccer_goal.is_ball_in_goal(self._soccer_ball)
        if ob_info["task/success"]:
            reward += 2.0  # overwrite reward on success

        if self._mocap_coordinate_marker is not None:
            pos = np.array([ob["soccer_ball/position"][0], ob["soccer_ball/position"][1] - 0.09, 0.05])
            self._mocap_coordinate_marker.move(create_transformation_matrix(translation=pos))
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        if self._initial_position_noise:
            ...
            ball_position = extract_position(self._soccer_ball.body_pose()[0])
            initial_noise = rand_min_max(min_val=np.array([-0.15, -0.15, 0.05]), max_val=np.array([0.15, -0.1, 0.075]))
            pos = ball_position + initial_noise
        ...
        self._initial_ball_position = ob["soccer_ball/position"][:2]
    ...
    def _build_observation_space(
        ...
        object_observation_space = {
            "soccer_ball/position": gymnasium.spaces.Box(
                low=self._soccer_ball._ball_min_position,
                high=self._soccer_ball._ball_max_position,
                shape=(3,),
                dtype=np.float32,
            ),
        }
        ...
        observation_space.update(
            {
                "robot/distance_to_ball": gymnasium.spaces.Box(0.0, np.inf, shape=(1,), dtype=np.float32),
            }
        )
