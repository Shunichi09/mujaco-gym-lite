...
def _object_obervation(
    hinged_box: HingedBox, qpos_bounds
) -> tuple[dict[str, npt.NDArray], dict[str, list[npt.NDArray]]]:
    hinged_box_qpos = np.array(hinged_box.joint_qpos(), dtype=np.float32).reshape((1,))
    orig_qpos = hinged_box_qpos.copy()
    ...
    if not np.array_equal(orig_qpos, clipped_qpos):
        logger.debug(f"Hinged box position clipped:\n  before={orig_qpos}\n  after ={clipped_qpos}")

    object_observation = {
        "hinged_box/qpos": clipped_qpos,
        "hinged_box/handle_position": np.array(hinged_box.handle_center_position(), dtype=np.float32),
    }
...
def _env_observation(
    ...
    hinged_box_qpos_min: npt.NDArray,
    hinged_box_qpos_max: npt.NDArray,
    # object obs settings
    ...
    object_obs, object_info = _object_obervation(
        hinged_box=hinged_box, qpos_bounds=[hinged_box_qpos_min, hinged_box_qpos_max]
    )
    ...
    robot_special_obs = {
        "robot/distance_to_handle_center": np.array(
            [hinged_box.distance_to_handle_center(extract_position(robot.end_effector_pose()))], dtype=np.float32
        ),
    }
...
def _compute_reward(
    *,
    ...
    handle_qpos: float,
    target_handle_qpos: float,
    initial_end_effector_position: npt.NDArray,
):
    """compute reward of hinged box opening task"""
    assert target_handle_qpos < 0.0
    if target_handle_qpos - handle_qpos > 0.0:
        handle_qpos = target_handle_qpos

    handle_error = np.abs(handle_qpos - target_handle_qpos)
    reward_for_opening = tolerance(
        handle_error,
        bounds=(0, 0.01),
        margin=np.deg2rad(45.0),
        sigmoid="long_tail",
    )

    target_position = handle_position - np.array([0.015, 0.0125, 0.0])
    gripper_error = (target_position - end_effector_position) * np.array([1.0, 1.0, 3.0])

    reward_for_caging = tolerance(
        float(np.linalg.norm(gripper_error)),
        bounds=(0.0, 0.03),  # target range
        margin=0.3,  # margin for decays
        sigmoid="long_tail",  # decay function shape
    )
    reward = 3.0 * reward_for_caging + 4.0 * reward_for_opening
    return float(reward), {
...
def _step_to_target(robot, model, data, frame_skip, target_pos, target_quat, max_trans_step=0.05, render_fn=None):
    for _ in range(100):
        ...
        robot.apply_gripper_control(np.array([100.0, 100.0]))
        robot_action(
            robot,
            ...
            open=False,
            home=False,
            gripper_command=np.array([100.0, 100.0]),  # always close
        )
...
class HingedBoxRobotEnv(MujocoEnv):
    metadata = {
    ...
    def __init__(
        ...
        self._hinged_box.load_mj_model_and_data(self.model, self.data)
    ...
    def step(self, action: Any):
        ...
        robot_action(
            ...
            gripper_command=np.array([100.0, 100.0]),
            open=False,
            close=False,
        ...
        ob, ob_info = _env_observation(
            ...
            hinged_box_qpos_min=self.observation_space["hinged_box/qpos"].low,
            hinged_box_qpos_max=self.observation_space["hinged_box/qpos"].high,
            hinged_box=self._hinged_box,
            robot=self._robot,
        ...
        reward, reward_info = _compute_reward(
            handle_qpos=ob["hinged_box/qpos"][0],
            target_handle_qpos=-np.deg2rad(45.0),
            handle_position=ob["hinged_box/handle_position"],
        ...
        ob_info["task/success"] = self._hinged_box.is_open(target_angle=-np.deg2rad(45.0))
        ...
        if self._mocap_coordinate_marker is not None:
            self._mocap_coordinate_marker.move(
                create_transformation_matrix(
                    translation=ob["hinged_box/handle_position"] - np.array([0.0125, 0.0125, 0.0])
                )
            )
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        if self._initial_position_noise:
            ...
            # hinged_box: np.array([0.1, -0.08, 0.125], dtype=np.float32)
            ee_pose = self._robot.end_effector_pose()
            hinged_box_handle_pos = self._hinged_box.handle_center_position()
            initial_noise = rand_min_max(min_val=np.array([0.0, -0.025, 0.0]), max_val=np.array([0.2, 0.025, 0.1]))
            pos = hinged_box_handle_pos + initial_noise
        ...
        self._initial_handle_position = ob["hinged_box/handle_position"]
    ...
    def observation_info_keys(self) -> tuple[str, ...]:
        ...
        keys.append("hinged_box/contact_positions/handle")
        keys.append("hinged_box/contact_names/handle")
    ...
    def _build_observation_space(
        ...
        assert self._hinged_box._hinged_box_joint_range[0] < self._hinged_box._hinged_box_joint_range[1]
        object_observation_space = {
            "hinged_box/qpos": gymnasium.spaces.Box(
                low=self._hinged_box._hinged_box_joint_range[0],
                high=self._hinged_box._hinged_box_joint_range[1],
                shape=(1,),
                dtype=np.float32,
            ),
            "hinged_box/handle_position": gymnasium.spaces.Box(-np.inf, np.inf, shape=(3,), dtype=np.float32),
        }
