...
from mujaco_gym_lite.environment_tools.mujoco_env.builders.hinged_box import (
...
def generate_path(robot_position: npt.NDArray) -> tuple[npt.NDArray, npt.NDArray]:
    handle_x = 0.005
    handle_y = -0.01 + 0.1
    handle_height = 0.15
    ...
    stay1_position = np.array([handle_x - 0.0, handle_y - 0.15, handle_height], dtype=np.float32)
    stay1_rotation = combine_quat(init_rotation, euler_to_quat(np.pi * 0.0, order="z"))

    stay2_position = np.array([handle_x - 0.1, handle_y - 0.2, handle_height], dtype=np.float32)
    stay2_rotation = combine_quat(init_rotation, euler_to_quat(np.pi * 0.0, order="z"))

    end_position = np.array([handle_x - 0.35, handle_y - 0.45, handle_height], dtype=np.float32)
...
def run_env(output_dir_path: pathlib.Path, render_mode: str, record: bool):
    ...
    config = HingedBoxRobotEnvBuilderConfig(
        home_arm_qpos=np.array([-1.61779153, 3.81199834, 2.64718577, -1.06878415, 2.60586587, -2.68657568]),
    ...
    builder = HingedBoxRobotEnvBuilder(config)
    ...
    initial_camera_position = np.array([look_at_point[0] + 0.5, look_at_point[1] - 0.4, look_at_point[2]])
    ...
    robot_position_path, robot_rotation_path = generate_path(state["robot/end_effector/position"])
    ...
    while True:
        ...
        logger.info(f"Reward info: Reward {reward} / Site pos {next_state['hinged_box/handle_position']}")
        logger.info(
        ...
        target_state["hinged_box/handle_position"].append(next_state["hinged_box/handle_position"].tolist())

        if info["task/success"]:
            logger.info("Task Success!")
            logger.info(f"Hinged box handle position: {next_state['hinged_box/handle_position']}")
...
def main():
    ...
    result_dir = pathlib.Path(__file__).parent / "demo_result" / "hinged_box_open"
