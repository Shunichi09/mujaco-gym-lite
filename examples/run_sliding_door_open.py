...
from mujaco_gym_lite.environment_tools.mujoco_env.builders.door import (
...
def generate_path() -> tuple[npt.NDArray, npt.NDArray]:
    ...
    init_position = np.array([0.15, 0.0, 0.45], dtype=np.float32)
    ...
    middle_position = np.array([window_x, 0.135, 0.3], dtype=np.float32)
    middle_rotation = init_rotation

    stay_position = np.array([window_x, depth, 0.3], dtype=np.float32)
    stay_rotation = init_rotation

    end_position = np.array([window_x - 0.275, depth, 0.3], dtype=np.float32)
    end_rotation = init_rotation

    position = _plan_linear_path(
        [init_position, middle_position, stay_position, end_position, end_position],
        num_points_between_way_points=[60, 30, 60, 20],
    )
    rotation = _plan_rotation_path(
        [init_rotation, middle_rotation, stay_rotation, end_rotation, end_rotation],
        num_points_between_way_points=[60, 30, 60, 20],
    )
...
def run_env(output_dir_path: pathlib.Path, render_mode: str, record: bool):
    ...
    config = SlidingDoorRobotEnvBuilderConfig(
        home_arm_qpos=np.array([-0.8919005, 3.44780644, 1.20367211, -2.51766909, 1.07256365, -0.9557543]),
    ...
    builder = SlidingDoorRobotEnvBuilder(config)
    ...
    while True:
        ...
        logger.info(f"Reward info: Reward {reward}, next_state {next_state['robot/distance_to_handle_center']}")
        target_state["robot/end_effector/position"].append(next_state["robot/end_effector/position"].tolist())
        target_state["door/handle_position"].append(next_state["door/handle_position"].tolist())

        if bool(next_state["robot/touch_handle"]):
            logger.info(f"{next_state['robot/touch_handle']}")
        if info["task/success"]:
            logger.info("Task Success!")
            logger.info(f"Door handle position: {next_state['door/handle_position']}")
...
def main():
    ...
    result_dir = pathlib.Path(__file__).parent / "demo_result" / "sliding_door"
