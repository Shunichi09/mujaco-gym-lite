...
from mujaco_gym_lite.environment_tools.mujoco_env.builders.lever import LeverRobotEnvBuilder, LeverRobotEnvBuilderConfig
...
def generate_path(robot_position: npt.NDArray, handle_position: npt.NDArray) -> tuple[npt.NDArray, npt.NDArray]:
    handle_x = handle_position[0]
    handle_y = handle_position[1]
    handle_height = handle_position[2]

    init_position = robot_position
    init_rotation = combine_quat(
        np.array([0.0, 0.70710678, 0.70710678, 0.0], dtype=np.float32),
        euler_to_quat(np.pi, order="z"),
    )

    middle_position = np.array([handle_x, handle_y, handle_height], dtype=np.float32)
    middle_rotation = combine_quat(init_rotation, euler_to_quat(np.pi * 0.0, order="z"))

    stay1_position = np.array([handle_x, handle_y - 0.1, handle_height - 0.1], dtype=np.float32)
    stay1_rotation = combine_quat(init_rotation, euler_to_quat(np.pi * 0.0, order="z"))

    stay2_position = np.array([handle_x, handle_y - 0.2, handle_height - 0.1], dtype=np.float32)
    stay2_rotation = combine_quat(init_rotation, euler_to_quat(np.pi * 0.0, order="z"))

    end_position = np.array([handle_x, handle_y - 0.25, handle_height - 0.15], dtype=np.float32)
...
def run_env(output_dir_path: pathlib.Path, render_mode: str, record: bool):
    ...
    config = LeverRobotEnvBuilderConfig(
        home_arm_qpos=np.array([-1.64991198, 3.6542934, 2.3370847, -0.84211868, 2.30209756, 0.64956199]),
    ...
    builder = LeverRobotEnvBuilder(config)
    ...
    robot_position_path, robot_rotation_path = generate_path(
        state["robot/end_effector/position"], state["lever/handle_position"]
    )
    ...
    while True:
        ...
        logger.info(f"Reward info: Reward {reward} / Site pos {next_state['lever/handle_position']}")
        logger.info(
            f"Action: {action['robot/end_effector/position']} State: {next_state['robot/end_effector/position']}"
        )
        logger.info(
            f"Info: {info['reward/reward_for_caging']} / {info['reward/handle_error']} / "
            f"{info['reward/reward_for_opening']}"
        )
        target_state["robot/end_effector/position"].append(next_state["robot/end_effector/position"].tolist())
        target_state["lever/handle_position"].append(next_state["lever/handle_position"].tolist())

        if info["task/success"]:
            logger.info("Task Success!")
            logger.info(f"Hinged box handle position: {next_state['lever/handle_position']}")
...
def main():
    ...
    result_dir = pathlib.Path(__file__).parent / "demo_result" / "lever_pull"
