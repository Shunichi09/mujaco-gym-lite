...
from mujaco_gym_lite.environment_tools.mujoco_env.builders.lidded_box import (
...
def generate_path(robot_position: npt.NDArray, lid_position: npt.NDArray) -> tuple[npt.NDArray, npt.NDArray]:
    target_x = 0.0
    target_y = 0.0
    target_z = 0.08
    ...
    middle_position = np.array([target_x, target_y, init_position[2]], dtype=np.float32)
    middle_rotation = combine_quat(init_rotation, euler_to_quat(np.pi * 0.0, order="z"))

    end_position = np.array([target_x, target_y, target_z], dtype=np.float32)
    end_rotation = combine_quat(init_rotation, euler_to_quat(np.pi * 0.0, order="z"))

    position = _plan_linear_path(
        [init_position, middle_position, end_position],
        num_points_between_way_points=[50, 50],
    )
    rotation = _plan_rotation_path(
        [init_rotation, middle_rotation, end_rotation],
        num_points_between_way_points=[50, 50],
    )
...
def run_env(output_dir_path: pathlib.Path, render_mode: str, record: bool):
    ...
    config = LiddedBoxRobotEnvBuilderConfig(
        home_arm_qpos=np.array([-1.04906454, 3.8513547, 1.68162966, -0.30987864, 1.13917531, -4.50053584]),
    ...
    builder = LiddedBoxRobotEnvBuilder(config)
    ...
    robot_position_path, robot_rotation_path = generate_path(
        state["robot/end_effector/position"], state["lid/position"]
    )
...
def main():
    ...
    result_dir = pathlib.Path(__file__).parent / "demo_result" / "lidded_box_reach"
