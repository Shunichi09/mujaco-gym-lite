...
from mujaco_gym_lite.environment_tools.mujoco_env.builders.drawer import (
...
def generate_path() -> tuple[npt.NDArray, npt.NDArray]:
    ...
    init_position = np.array([0.0, 0.025, 0.45], dtype=np.float32)
    init_rotation = np.array([0.0, 0.70710678, 0.70710678, 0.0], dtype=np.float32)

    middle_position = np.array([drawer_x, -0.08, 0.16], dtype=np.float32)
    middle_rotation = combine_quat(init_rotation, euler_to_quat(np.pi * 0.0, order="z"))

    stay_position = np.array([drawer_x, -0.08, height], dtype=np.float32)
    stay_rotation = combine_quat(init_rotation, euler_to_quat(np.pi * 0.0, order="z"))

    end_position = np.array([drawer_x, -0.275, height], dtype=np.float32)
    end_rotation = combine_quat(init_rotation, euler_to_quat(np.pi * 0.0, order="z"))
    position = _plan_linear_path(
        [init_position, middle_position, stay_position, end_position], num_points_between_way_points=[70, 20, 70]
    )
    rotation = _plan_rotation_path(
        [init_rotation, middle_rotation, stay_rotation, end_rotation], num_points_between_way_points=[70, 20, 70]
    )
...
def run_env(output_dir_path: pathlib.Path, render_mode: str, record: bool):
    ...
    config = DrawerRobotEnvBuilderConfig(
    ...
    builder = DrawerRobotEnvBuilder(config)
    ...
    while True:
        ...
        target_state["drawer/handle_position"].append(next_state["drawer/handle_position"].tolist())

        if bool(next_state["robot/touch_handle"]):
            logger.info(f"{next_state['robot/touch_handle']}")
        if info["task/success"]:
            logger.info("Task Success!")
            logger.info(f"Drawer handle position: {next_state['drawer/handle_position']}")
...
def main():
    ...
    result_dir = pathlib.Path(__file__).parent / "demo_result" / "drawer"
